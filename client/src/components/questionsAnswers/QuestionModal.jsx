...
const QuestionModal = ({modal, setModal, productName, productid}) => {
  ...
  return (
    <>
      <div role="question-modal" className="modal">
        <div onClick={toggleModal} className="w-100vw h-100vh top-0 left-0 right-0 bottom-0 fixed bg-neutral-600 opacity-70 "></div>
        <div className="fixed top-36 right-2/4 translate-x-2/4 leading-6 bg-neutral-50 border-4 rounded w-96 h-124">
          ...
          <form className="flex flex-col" onSubmit={(e) => {
            ...
            <textarea maxLength="1000" required className="m-2 h-48 resize-none" placeholder="Your question"></textarea>
            <input maxLength="60" required className="m-2" placeholder="Example: jackson11!"></input>
            <span className="m-2 text-xs">For privacy reasons, do not use your full name or email address</span>
            <input maxLength="60" required type="email" className="m-2" placeholder="Example: jack@email.com"></input>
